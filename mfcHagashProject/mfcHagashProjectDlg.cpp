...
BOOL CmfcHagashProjectDlg::OnInitDialog()
{
	...
	isErased=false;
	currFigure=Line;
	m_selectCL=RGB(rand()%255,rand()%255,rand()%255); //first color is RANDOM
	m_colorCTRL.SetColor(m_selectCL);
	m_selectCL2=RGB(rand()%255,rand()%255,rand()%255); // SAME 
	...
	while(pwndChild)
	{
		...
		// Changes the Child Rect by the values of  offset
		rcChild.OffsetRect(ptOffset); 
		pwndChild->MoveWindow(rcChild,FALSE); // Move the Child Control
		pwndChild = pwndChild->GetNextWindow();
	}
	CRect	rcWindow;
	GetWindowRect(rcWindow); //Increase width to new Client Width
	rcWindow.right += rcClientOld.Width() - rcClientNew.Width(); // Increase height
	rcWindow.bottom += rcClientOld.Height() - rcClientNew.Height(); 
	MoveWindow(rcWindow,FALSE); // Redraw Wind
...
{ //Mouse Move
	...
	CPen pen;//Add Pen 
	CBrush brush;//Add Brush 
	CClientDC dc(this);//Add Client Dc (check?)
	dc.SetROP2(R2_NOTXORPEN);
	

	if(isPressed)//if isPressed draw
	{
...
void CmfcHagashProjectDlg::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	start=end=point;
	if(deleteOBJ) // Edit mode can delete and change color 
	{
		deleteOBJ=false;
...
BOOL CmfcHagashProjectDlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	...
	if(pWnd==this)
		m_stat.SetPaneText(2,L"AlPaint");
	else
	{
